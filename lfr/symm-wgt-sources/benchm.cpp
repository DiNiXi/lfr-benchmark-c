...
double* julia_vector_return( deque<set<int> > & E, const deque<deque<int> > & member_list, deque<map <int, double > > & neigh_weigh) {
	...
	double * ret = (double *) malloc( (3*edges+num_nodes+2) * sizeof(double) );

	ret[0] = num_nodes; ret[1] = edges;
...
double* benchmark(bool excess, bool defect, int num_nodes,
				  double  average_k,
				  int  max_degree,
				  ...
				  int  nmin, int  nmax,
				  bool  fixed_range, double ca, int seed) {
	...
	if(ca!=unlikely)
		cout<<"Average clustering coefficient: "<<ca<<endl;
	...
	return julia_vector_return( E, member_list, neigh_weigh );
