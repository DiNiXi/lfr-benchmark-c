...
int* julia_vector_return( deque<set<int> > & E, const deque<deque<int> > & member_list) {
	...
	int * ret = (int *) malloc( (2*edges+num_nodes+2) * sizeof(int) );
...
int* benchmark(bool excess, bool defect, int num_nodes,
			   double  average_k,
			   int  max_degree,
	...
	return julia_vector_return( Eout, member_list );
